...
//Code Here
let totalPopulation = populations.reduce((sum,current)=> sum+current);
...
*/
let bobsArray = purchases.filter(e => {
  if(e.owner === "Bob"){
    return e.owner;
  }
})
console.log(bobsArray);
let totalCollecter = (arr)=> {
  let prices = [];
...
console.log(totalCollecter);
let bobsTotal = totalCollecter(bobsArray).reduce((sum,current)=> sum + current);
